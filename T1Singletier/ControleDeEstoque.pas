...
var
  vCampos , cont:integer;
begin
cont:=0;
for vCampos := 0 to Pred(ds1.FieldCount) do
if not ds1.Fields[vCampos].IsNull then
Inc(cont);
cbbAcao := 'Indexar';
cbbAcao := 'Limpar';
...
begin
   Validar se todos os campor foram preenchidos(menos os aggregates)
    casa algum não foi preenchido vc deve abordar aa edição
end; }
